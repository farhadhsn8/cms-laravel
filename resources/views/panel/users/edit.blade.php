        ...
        <div class="row no-gutters bg-white margin-bottom-20">
            <div class="col-12">
                ...
                <form action="{{route('users.update',$user->id)}}" class="padding-30" method="post">
                    @csrf
                    @method('put')
                    <input type="text" value="{{$user->name}}" name="name" class="text" placeholder="نام و نام خانوادگی">
                    @error('name')
                    <p class="error">{{$message}}</p>
                    @enderror
                    <input type="text" value="{{$user->email}}" name="email" class="text" placeholder="ایمیل">
                    @error('email')
                    <p class="error">{{$message}}</p>
                    @enderror
                    <input type="text" value="{{$user->mobile}}" name="mobile" class="text" placeholder="شماره موبایل">
                    @error('mobile')
                    <p class="error">{{$message}}</p>
                    @enderror
                    <select name="role" >
